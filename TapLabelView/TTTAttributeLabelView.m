...
- (void)changeCurrentLinkAttributeWith:(NSRange)range URL:(NSURL *)url color:(UIColor *)color {
    NSDictionary *dic = @{nsStringWithCfString(kCTForegroundColorAttributeName) : color,
                          nsStringWithCfString(kCTUnderlineStyleAttributeName)  : [NSNumber numberWithInt:[self checkEnumValueValid:self.linkUnderLineStyle]]};
    [self.label replaceLinkWithRang:range URL:url attributes:dic];
}
